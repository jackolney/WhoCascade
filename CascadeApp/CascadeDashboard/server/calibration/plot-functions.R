...
BuildCalibrationPlots <- function(data, originalData) {
    ...
    p6 <- p6 + geom_point(data = originalData[["calib"]][originalData[["calib"]]$year == 2015 & originalData[["calib"]]$indicator != "PLHIV Retained",], aes(x = indicator, y = value), color = "black", size = 2)
