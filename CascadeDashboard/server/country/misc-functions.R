AddNAToMasterData <- function(theBlank, theData) {
    ...
    for(i in 1:length(uInd)) {
        for(j in 1:length(uYr)) {
            if (isEmpty(theData[theData$indicator    == uInd[i] & theData$year  == uYr[j], "value"])) next else {
            ...
            if (isEmpty(theData[theData$indicator    == uInd[i] & theData$year  == uYr[j], "weight"])) next else {
